...
function alertPlugin(md) {
    ...
    md.core.ruler.after('block', 'github_alerts', (state) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (inlineToken.children) {
                ...
                const firstNonEmpty = inlineToken.children.findIndex((c) => c.content !== '' || (c.type !== 'text' && c.type !== 'softbreak'));
                if (firstNonEmpty > 0) {
    ...
    md.renderer.rules.blockquote_open = (tokens, idx, options, env, self) => {
        ...
        const alertType = token.meta?.alertType;
        if (alertType) {
