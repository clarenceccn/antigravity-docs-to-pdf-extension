...
function resolveExportPath() {
    const config = vscode.workspace.getConfiguration('antigravity');
    const exportPath = config.get('exportPath', 'docs');
 ...
 */
function findArtifacts(brainPath) {
    const results = [];
 ...
 */
function buildOutputPath(exportDir, conversationId, filename) {
...
async function exportAll() {
    const config = vscode.workspace.getConfiguration('antigravity');
    const brainPath = expandHome(config.get('brainPath', '~/.gemini/antigravity/brain'));
    if (!fs.existsSync(brainPath)) {
        vscode.window.showErrorMessage(`Brain directory not found: ${brainPath}\n\nConfigure it via Settings → antigravity.brainPath`);
        return;
    }
    const artifacts = findArtifacts(brainPath);
    if (artifacts.length === 0) {
        vscode.window.showWarningMessage(`No artifacts found in ${brainPath}. Looking for: ${ARTIFACT_FILES.join(', ')}`);
        return;
    }
    ...
    await vscode.window.withProgress({
        ...
        const generator = new PdfGenerator_js_1.PdfGenerator(browserPool);
        let completed = 0;
        const errors = [];
        ...
        if (errors.length > 0) {
            vscode.window.showErrorMessage(`Exported ${completed}/${total} PDFs with ${errors.length} error(s):\n${errors.join('\n')}`);
        }
        else {
            const action = await vscode.window.showInformationMessage(`Exported ${completed} PDF(s) to ${exportDir}`, 'Open Folder');
 ...
 */
async function resolveArtifactFromTabLabel(tabLabel) {
    if (!tabLabel)
        return undefined;
...
async function exportCurrentFile(uri) {
    ...
    if (!filePath.endsWith('.md')) {
        const proceed = await vscode.window.showWarningMessage('The current file is not a Markdown file. Export anyway?', 'Export', 'Cancel');
        if (proceed !== 'Export')
            return;
    }
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const action = await vscode.window.showInformationMessage(`PDF saved: ${path.basename(result)}`, 'Open PDF', 'Open Folder');
            if (action === 'Open PDF') {
...
function createStatusBarButton(context) {
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(() => updateStatusBarVisibility()), vscode.window.tabGroups.onDidChangeTabs(() => updateStatusBarVisibility()));
}
...
function activate(context) {
    ...
    browserPool = new browserPool_js_1.BrowserPool(getChromePath());
    // Ensure the browser pool is disposed when the extension deactivates
    context.subscriptions.push({ dispose: () => browserPool.dispose() });
    context.subscriptions.push(vscode.commands.registerCommand('antigravity.exportAll', exportAll), vscode.commands.registerCommand('antigravity.exportCurrentFile', exportCurrentFile));
